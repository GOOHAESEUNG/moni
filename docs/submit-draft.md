...
- Grossman et al.(CHI 2025): "Playing Dumb to Get Smart" — LLM이 멍청한 척할 때 학생 능동적 학습 유도 실증
- BEA 2025 "LLMs Protégés": 지식 격차 LLM 가르치기로 중간고사 실패율 72% 감소
- Google LearnLM RCT(2025, 165명): AI 튜터 성공률 66.2% > 인간 튜터 60.7%
- MathSpring RCT(2025, 2,003명): AI 튜터 사용 그룹 유의미한 수학 성취 향상
- 게이미피케이션 메타분석(2023, 41개 연구): 학습 효과 크기 g=0.822

**확장 가능성:**
1. **다과목 확장**: 교육과정 JSON 구조화로 수학 외 영어/과학/사회 즉시 추가 가능
2. **다학년 확장**: gradeHint 파라미터로 학년별 어휘 수준 자동 조절
3. **학부모 연동**: 리포트 공유 링크 → 학부모 앱 없이도 모바일에서 확인
4. **SaaS 전환**: 현재 단일 학원 → 멀티테넌트 구조로 확장 (Supabase RLS 이미 적용)
5. **파인튜닝 고도화**: AI-Hub 외 실제 서비스 데이터 누적 → 역량 분석 정확도 향상
